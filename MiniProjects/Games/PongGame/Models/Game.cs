...
namespace CsharpMiniProjects.MiniProjects.Games.PongGame.Models
{
    public class Game
    {
        private SoundPlayer paddleHitSound;
        private SoundPlayer ArcadeGoalSound;
        private SoundPlayer WallTapSound;
                    ...
                    Rectangle rightWallTop, Rectangle rightWallBottom)
        {
            ...
            // Load the sound file
            paddleHitSound = new SoundPlayer(Application.GetResourceStream(
                new Uri("pack://application:,,,/MiniProjects/Games/PongGame/Sounds/hit.wav")).Stream);
            paddleHitSound.Load();

            ArcadeGoalSound = new SoundPlayer(Application.GetResourceStream(
                new Uri("pack://application:,,,/MiniProjects/Games/PongGame/Sounds/ArcadeGoal.wav")).Stream);
            ArcadeGoalSound.Load();

            WallTapSound = new SoundPlayer(Application.GetResourceStream(
                new Uri("pack://application:,,,/MiniProjects/Games/PongGame/Sounds/WallTapSoundQuiet.wav")).Stream);
            WallTapSound.Load();
        }
        ...
        private void CorrectBallPaddleOverlap()
        {
            ...
            {
                Ball.InvertSpeedX();
                paddleHitSound.Play();  // Add this line to play the sound

                
                double hitPosition = (Ball.Y + Ball.Height / 2) - (paddleTop + Paddle1.Rectangle.Height / 2);
                double maxAngle = 2;
        ...
        private void CheckCollisions()
        {
            ...
            // Calculate expanded collision area when spring is active
            double paddle1HitboxExpansion = Paddle1.IsSpringing ? 20 : 0; // Increase the paddle's effective height by 20 units during spring
            double paddle2HitboxExpansion = Paddle2.IsSpringing ? 20 : 0;

            // Ball collision with Paddle1 (Player 1)
            if (Ball.X <= Paddle1.X + Paddle1.Rectangle.Width &&
                Ball.Y + Ball.Height >= Paddle1.Y - paddle1HitboxExpansion &&
                Ball.Y <= Paddle1.Y + Paddle1.Rectangle.Height + paddle1HitboxExpansion)
            {
                
                Ball.SpeedX = Math.Abs(Ball.SpeedX); // Reflect the ball
                paddleHitSound.Play();
                ...
                {
                    Ball.SpeedX *= Paddle1.SpringSpeedBoost; // Boost ball speed
                    Ball.SpeedY *= Paddle1.SpringSpeedBoost;
                }
            }

            // Ball collision with Paddle2 (Player 2)
            if (Ball.X + Ball.Width >= Paddle2.X &&
                Ball.Y + Ball.Height >= Paddle2.Y - paddle2HitboxExpansion &&
                Ball.Y <= Paddle2.Y + Paddle2.Rectangle.Height + paddle2HitboxExpansion)
            {
                Ball.SpeedX = -Math.Abs(Ball.SpeedX); // Reflect the ball
                paddleHitSound.Play();
                ...
                {
                    Ball.SpeedX *= Paddle2.SpringSpeedBoost; // Boost ball speed
